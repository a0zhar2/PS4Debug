// ---------------------------------------
// PS4Debug (PS4 Debugger) coded by Golden
// For System FW 6.72
// -----
// Fork maintained by A0zhar
// ---------------------------------------
#pragma once
#ifndef _RPCASM_H
#define _RPCASM_H

#include <stdint.h>

#define RPCSTUB_MAGIC 0x42545352
#define RPCLDR_MAGIC  0x52444C52

// Structure of RPC (Remote Procedure Call) stub header
struct rpcstub_header {
    uint32_t magic; // Magic number
    uint64_t entry; // Entry point
    uint64_t rpc_rip; // RIP register value for RPC
    uint64_t rpc_rdi; // RDI register value for RPC
    uint64_t rpc_rsi; // RSI register value for RPC
    uint64_t rpc_rdx; // RDX register value for RPC
    uint64_t rpc_rcx; // RCX register value for RPC
    uint64_t rpc_r8; // R8 register value for RPC
    uint64_t rpc_r9; // R9 register value for RPC
    uint64_t rpc_rax; // RAX register value for RPC
    uint8_t rpc_go; // Flag indicating RPC start
    uint8_t rpc_done; // Flag indicating RPC completion
} __attribute__((packed));

// Structure of RPC (Remote Procedure Call) loader header
struct rpcldr_header {
    uint32_t magic; // Magic number
    uint64_t entry; // Entry point
    uint8_t ldrdone; // Flag indicating loader completion
    uint64_t stubentry; // Stub entry point
    uint64_t scePthreadAttrInit; // Address of pthread_attr_init
    uint64_t scePthreadAttrSetstacksize; // Address of pthread_attr_setstacksize
    uint64_t scePthreadCreate; // Address of pthread_create
    uint64_t thr_initial; // Initial thread function address
} __attribute__((packed));

// RPC (Remote Procedure Call) stub binary data
static const uint8_t rpcstub[410] = {
    
};

// RPC (Remote Procedure Call) loader binary data
static const uint8_t rpcldr[255] = {
    
};

#endif
