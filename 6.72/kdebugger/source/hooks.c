...
// Function to write an absolute jump to the specified address
...
int sys_proc_list(struct thread *td, struct sys_proc_list_args *uap) {
    ...
    if (!uap->procs) {
        ...
        *uap->num = process_count;
    } else {
        p = *allproc;
        // Fill structure with process info
        for (int i = 0; i < *uap->num && p; i++) {
...
// Function that allows for read/write operation's to process memory
...
// Function to allocate memory in a process
...
// Function to free allocated memory in a process
...
// Function to get virtual memory mappings of a process
int sys_proc_vm_map_handle(struct proc *p, struct sys_proc_vm_map_args *args) {
    ...
    if (!args->maps) {
        ...
        for (int i = 0; i < args->num && entry; i++) {
...
// Function to install a hook in a process
...
// Function to call a hook in a process
