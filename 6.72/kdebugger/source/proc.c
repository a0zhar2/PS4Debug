...
// Function to perform memory read/write operation for a Process
int proc_rw_mem(struct proc *p, void *ptr, uint64_t size, void *data, uint64_t *n, int write) {
    // Make sure that the <proc> struct passed as <p> is valid.
    // If not, return early the code 1
    if (!p) return 1;
    
    struct thread *thread_descriptor = curthread();
    ...
    // Clear iovector memory from junk
    memset(&ioVector, NULL, sizeof(struct iovec));
    ioVector.iov_base = (uint64_t)data;
    ioVector.iov_len = size;

    // Clear uio memory from junk
    memset(&uio, NULL, sizeof(struct uio));
    uio.uio_iov = (uint64_t)&ioVector;
    uio.uio_iovcnt = 1;
    uio.uio_offset = (uint64_t)ptr;
    uio.uio_resid = (uint64_t)size;
    uio.uio_segflg = UIO_SYSSPACE;
    uio.uio_rw = write ? UIO_WRITE : UIO_READ;
    uio.uio_td = thread_descriptor;
...
// Function to Read <size> number of bytes from the memory of the Process specified by
// the <p> proc struct instance
inline int proc_read_mem(struct proc *p, void *ptr, uint64_t size, void *data, uint64_t *n) {
    return proc_rw_mem(p, ptr, size, data, n, 0);
}

// Function to Write <size> number of bytes to the memory of the Process specified by
// the <p> proc struct instance
